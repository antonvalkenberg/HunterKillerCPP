...
public:
    ...
    bool operator!=(const MapLocation& rLocation) const;
    [[nodiscard]] bool Equals(const MapLocation& rLocation) const;
    [[nodiscard]] int GetHashCode() const { return 41 * X + 53 * Y; }
    [[nodiscard]] std::string ToString() const { return std::format("[{0:d},{1:d}]", X, Y); }
